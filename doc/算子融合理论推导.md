...
#### 总结
算子优化时可以根据$MNK$的大小，选择需要缓存最小的方案。比如flashattentionv1作者在计算$QK^{T}$时选取了$K->M->N$中第二个方案，flashattentionv2中，作者选取了方案$M->N->K$中的第二个方案，需要最小缓存为

### Tile乘法
分块矩阵相乘一般假设，缓存能够缓存下 $TileC, TiledA, TiledB$ 

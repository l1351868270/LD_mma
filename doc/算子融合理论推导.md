...
##### 无缓存
第一种情况是，系统中没有缓存。此时每个 $C$ 中的元素可以在寄存器中累加，计算好之后写入全局内存一次，即 $MN$ 次写。 $A, B$ 需要分别读取 $MNK$ 次
        $$C_w=MN$$
        $$A_r=MKN$$
        $$B_r=KNM$$
        $$total=2MNK+M*N$$
...
##### 可以缓存下 $A(i)$ 
第二种情况是，系统中有缓存但是比较小，只能缓存下 $A$ 的第 $i$ 行。此时 $C$ 需要写入全局内存一次。 $A[i]$ 可以缓存在缓存中，不需要像无缓存时重复读取,所有只需要整体读取一次。 $B$ 在缓存中缓存不下，所以 $A$ 的每行都需要整体读取一次 $B$, 即读取 $MNK$ 次
        $$C_w=MN$$
        $$A_r=MK$$
        $$B_r=KNM$$
        $$total=MNK+MN+MK$$
...
##### 可以缓存下 $A(i)$ 和整个 $B$ 
第三种情况是，系统中有缓存并且是比较大，能缓存下 $A$ 的第 $i$ 行和整个 $B$ 。此时 $C$ 需要写入全局内存一次。 $A[i]$ 和 $B$ 可以缓存在缓存中，不需要像无缓存时重复读取,所有只需要整体读取一次。即读取 $MK+KN$ 次
        $$C_w=MN$$
        $$A_r=MK$$
        $$B_r=KN$$
        $$total=MN+MK+KN$$
...
##### 可以缓存下 $B(j)$ 
第二种情况是，系统中有缓存但是比较小，只能缓存下 $B$ 的第 $j$ 列。此时 $C$ 需要写入全局内存一次。 $B[j]$ 可以缓存在缓存中，不需要像无缓存时重复读取,所有只需要整体读取一次。 $A$ 在缓存中缓存不下，所以 $B$ 的每列都需要整体读取一次 $A$, 即读取 $MNK$ 次
        $$C_w=MN$$
        $$A_r=MKN$$
        $$B_r=KN$$
        $$total=MNK+MN+KN$$
...
##### 可以缓存下整个 $A$ 和 $B(j)$ 
第三种情况是，系统中有缓存并且是比较大，能缓存下 $B$ 的第 $j$ 列和整个 $A$ 。此时 $C$ 需要写入全局内存一次。 $A[i]$ 和 $B$ 可以缓存在缓存中，不需要像无缓存时重复读取,所有只需要整体读取一次。即读取 $MK+KN$ 次
        $$C_w=MN$$
        $$A_r=MK$$
        $$B_r=KN$$
        $$total=MN+MK+KN$$

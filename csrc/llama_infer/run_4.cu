...
ncu -k rmsnorm -f --set full -o run_4 ./run_4 stories42M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"

ncu -k rmsnormV2 --csv --log-file run_4_rmsnorm.csv --cache-control=all --clock-control=base --metrics gpu__time_duration.sum ./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
python stat-csv.py run_4_rmsnorm.csv --kernels "rmsnormV2"
['rmsnormV2']
kernel, mean(us), std, med, num
rmsnormV2,  4.932,  0.179,  4.864,  325

ncu -k ropeV2 --csv --log-file run_4_ropeV2.csv --cache-control=all --clock-control=base --metrics gpu__time_duration.sum ./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
python stat-csv.py run_4_ropeV2.csv --kernels "ropeV2"
['ropeV2']
kernel, mean(us), std, med, num
ropeV2,  3.373,  0.181,  3.424,  1936

ncu -k residual_connectionV2 --csv --log-file run_4_residual_connectionV2.csv --cache-control=all --clock-control=base --metrics gpu__time_duration.sum ./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
python stat-csv.py run_4_residual_connectionV2.csv --kernels "residual_connectionV2"
['residual_connectionV2']
kernel, mean(us), std, med, num
residual_connectionV2,  2.217,  0.131,  2.208,  3936

ncu -k swigluV2 --csv --log-file run_4_swigluV2.csv --cache-control=all --clock-control=base --metrics gpu__time_duration.sum ./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
python stat-csv.py run_4_swigluV2.csv --kernels "swigluV2"
['swigluV2']
kernel, mean(us), std, med, num
swigluV2,  2.662,  0.110,  2.656,  1856

ncu -k multihead_attentionV2 --csv --log-file run_4_multihead_attentionV2.csv --cache-control=all --clock-control=base --metrics gpu__time_duration.sum ./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
python stat-csv.py run_4_multihead_attentionV2.csv --kernels "multihead_attentionV2"

nsys profile --stats=true ./run_4 llama2_7b_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
Inference for Llama-2 Transformer model in pure C 
nvcc -O3 -arch=sm_86 -o run_4 run_4.cu
./run_4 stories15M_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
./run_4  llama2_7b_fp16.bin -t 0.8 -n 256 -i "One day, Lily met a Shoggoth"
*/
...
__global__ void get_content_row(half *x, half* token_embedding_tableint, int *token, int dim) {
    for (int i = 0; i<dim; i++) {
        x[i] = *(token_embedding_tableint + (*token) * dim + i);
    }
}

half* forward(cublasHandle_t* handle, Transformer* transformer, int *token, int htoken, int pos) {
    ...
    // get_content_row<<<1,1>>>();
    // half* content_row = w->token_embedding_table + htoken * dim;
    // half* content_row_host = (half*)malloc(dim*sizeof(*x));
    // cudaMemcpy(content_row_host, content_row, dim*sizeof(*x), cudaMemcpyDeviceToHost);
    // printf("token: %d\n", token);
    // for (int i = 0; i < dim; i++) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        // attention rmsnorm
        // rmsnormV3<<<1,32>>>(s->xb, w->token_embedding_table, token, w->rms_att_weight + l*dim, dim);
        ...
        //     int rotn = i < kv_dim ? 2 : 1; // how many vectors? 2 = q & k, 1 = q only
        //     for (int v = 0; v < rotn; v++) {
        //         half* vec = v == 0 ? s->q : s->k; // the vector to rotate (query or key)
        //         half v0;
        //         cudaMemcpy((void*)&v0, &vec[i], 1 * sizeof(v0), cudaMemcpyDeviceToHost);
        //         half v1;
        //         cudaMemcpy((void*)&v1, &vec[i+1], 1 * sizeof(v1), cudaMemcpyDeviceToHost);
        //         half vel_i = v0 * fcr - v1 * fci;
        //         half vel_ii = v0 * fci + v1 * fcr;
        //         // vec[i]   = v0 * fcr - v1 * fci;
        //         // vec[i+1] = v0 * fci + v1 * fcr;
        //         cudaMemcpy((void*)&vec[i], &vel_i, 1 * sizeof(vel_i), cudaMemcpyHostToDevice);
        //         cudaMemcpy((void*)&vec[i+1], &vel_ii, 1 * sizeof(vel_ii), cudaMemcpyHostToDevice);
        //         // printf("vel_i: %f\n", (float)vel_i);
        //     }
        // }
        ...
        //     // get the query vector for this head
        //     half* q = s->q + h * head_size;
        //     // attention scores for this head
        //     half* att = s->att + h * p->seq_len;
        //     // iterate over all timesteps, including the current one
        //     for (int t = 0; t <= pos; t++) {
        //         // get the key vector for this head and at this timestep
        //         half* k = s->key_cache + loff + t * kv_dim + (h / kv_mul) * head_size;
        //         // calculate the attention score as the dot product of q and k
        //         half score = 0.0f;
        //         for (int i = 0; i < head_size; i++) {
        //             score += q[i] * k[i];
        ...
        //     for (int t = 0; t <= pos; t++) {
        //         // get the value vector for this head and at this timestep
        //         half* v = s->value_cache + loff + t * kv_dim + (h / kv_mul) * head_size;
        //         // get the attention weight for this timestep
        //         half a = att[t];
        //         // accumulate the weighted value into xb
        //         for (int i = 0; i < head_size; i++) {
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    while (pos < steps) {
        ...
        half* logits = forward(&handle, transformer, dtokens + pos, token, pos);
